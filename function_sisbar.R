...
matchBC<-function(virus_csv,seurat_cellbar,suffix){
  csvfile=read.csv(paste0(virus_dir,virus_csv),header=T)
  csvfile$Barcode=paste0(csvfile$Barcode,"-",suffix)
  Cell.BC=gsub("-1","",as.character(seurat_cellbar))
  cb_list=unique(csvfile$cell)
  Virus.BC=c()
  num=c()
  for(cb in Cell.BC){
    if(cb %in% cb_list){
      cfsub=subset(csvfile,cell==cb)
      vblist=sort(cfsub$Barcode)
      vbline=paste(vblist,collapse=";")
      neach=length(vblist)
      Virus.BC<-c(Virus.BC,vbline)
      num<-c(num,neach)
    }
    else{
      Virus.BC<-c(Virus.BC,NA)
      num<-c(num,0)
    }
  }
  matched_data=data.frame(Virus.BC,Cell.BC,num)
  return(matched_data)
}
...
barplot_stat<-function(x,clu,jac_sample,all_jac,times){
  idx<-which(clu==x)
  dat_tmp=t(do.call("cbind",jac_sample))
  dat<-dat_tmp[seq(idx,(500*length(clu)),by=length(clu)),times]
  dat<-data.frame(dat,check.names = F) %>% gather(time,similarity)
  dat_true<-data.frame(t(all_jac),check.names=F)[idx,times]
  dat_true<-data.frame(dat_true,check.names=F) %>% gather(time,similarity)
  p<-ggplot()+
...
tmp_dot_plot<-function(datsub){
  p1<-ggplot(datsub,aes(Cluster,Cluster2,label=Num))+
    geom_point(aes(size=jaccard,color=-log2(pvalue+0.0001)),alpha=0.5)+
    scale_size_area(max_size = 10)+
    geom_fit_text(aes(size=jaccard_result),show.legend = F,family="Arial")+
    #geom_fit_text(size=8,color="#333399",fontface="bold")+
    scale_colour_gradient(low="#FFCCFF",high="#CC0099")+
    labs(color=expression(-log2(pvalue+0.0001)),size="jaccard similarity",x="",y="",title="")+
    #scale_size(limits = c(0.0001,max(dat$jaccard)))+
    theme_bw()+theme(text=element_text(size=12),axis.text.x = element_text(angle = 90,vjust = 0.5))
  return(p1)
}
