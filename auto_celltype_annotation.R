feature_position<-c("OTX2","EN1","FGF8","STMN2","LMX1A","FOXA2","BARHL1","BARHL2","CRH","PLP1","OLIG1","OLIG2","OTP")
...
celltype_names<-c("Neuron&Nb","P_dien","P_dien_PLP1","P_meten","P_messenBP","P_messenFP","P_MHB")
celltype_names<-data.frame(celltype_names)
variable_names<-c("nnb_clusters","dien_clusters","dien_PLP1_clusters","hind_clusters","mBP_clusters","mFP_clusters","mhb_clusters")
...
auto_cell_annotation_vitro<-function(sdata,clustername="RNA_snn_res.0.8",clustername_auto="Cluster_name_auto_res0.8"){
  ...
  pct_data<-c()
  cnames=levels(sdata@meta.data[[clustername]])
  for(cm in cnames){
  ...
  #hind, OTX2-EN1+
  hind_clusters<-rownames(subset(pct_sub,OTX2_to_EN1<0.5 & exp_OTX<3.5))
  #mid,OTX2+EN1+
  mid_rownames<-!rownames(pct_sub) %in% c(dien_clusters,dien_PLP1_clusters,hind_clusters)
